...
async def virused(ctx, member : discord.Member):
    ...
    if "Purple Haze" in (role.name for role in ctx.message.author.roles):
        ...
        random_member = random.choice(server.members)
        
        if random.randint(0, 3) == 1:
        
            if random.randint(0, 2) == 1:
                embed = discord.Embed(title = "**{}** заразил вирусом **{}**.".format(ctx.message.author.name, random_member.name), description = "**Стенд потерял контроль и случайно заразил вирусом не того.**", color = 0xffff00)
                embed.set_image(url="https://cdn.discordapp.com/attachments/473403386599964672/487280196450385932/unknown.png")
                await bot.send_message(ctx.message.channel, embed=embed)
                
                virus = random_member.id
                virus_S = 0
                ...
                if virus_S != 0:
                    ...
                    if virus_S != 0:
                        ...
                        if virus_S != 0:
                            ...
                            virus = ""
                            mute_role = discord.utils.find(lambda r: r.name == 'В муте', ctx.message.server.roles)
                            await bot.add_roles(random_member, mute_role)
            else:
...
async def ubasha(ctx, member : discord.Member):
    ...
    if "Purple Haze" in (role.name for role in ctx.message.author.roles):
        ...
        if random.randint(0, 3) == 1:
            ...
            random_member = random.choice(server.members)
            embed = discord.Embed(title = "UBASHAAAA, {}!".format(random_member.name), description = "*«УБАШАААА, {}!»* **Стенд взбесился и начал атаковать всех подряд.**".format(random_member.name), color = 0xffff00)
            embed.set_image(url="https://cdn.discordapp.com/attachments/473403386599964672/487280196450385932/unknown.png")
            await bot.send_message(ctx.message.channel, embed=embed)
            ...
            mute_role = discord.utils.find(lambda r: r.name == 'В муте', ctx.message.server.roles)
            await bot.remove_roles(random_member, mute_role)
            
            
            random_member = random.choice(server.members)
            embed = discord.Embed(title = "UBASHAAAA, {}!".format(random_member.name), description = "*«УБАШАААА, {}!»* **Стенд успокоился и перестал атаковать.**".format(random_member.name), color = 0xffff00)
            embed.set_image(url="https://cdn.discordapp.com/attachments/473403386599964672/487280196450385932/unknown.png")
            await bot.send_message(ctx.message.channel, embed=embed)
