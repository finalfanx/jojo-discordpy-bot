...
async def zero(ctx, member : discord.Member):
    ...
    if "Gold Experience" in (role.name for role in ctx.message.author.roles):
        if "Requiem" not in (role.name for role in ctx.message.author.roles):
            return
        embed = discord.Embed(title = "**GORUDO EXPERIENCU NO NORI OKU**.".format(ctx.message.author.name, member.name), description = "*{} теперь не сможет юзать бота.*".format(member.name), color = 0xffff00)
...
async def aero(ctx, member : discord.Member, smember : discord.Member):
    ...
    if "Aerosmith" in (role.name for role in ctx.message.author.roles):
        ...
        if random.randint(0, 3) == 1:
            embed = discord.Embed(title = "VOLAVOLAVOLA, **{}**, **{}**.".format(member.name, smember.name), description = "*... VOLARE VIA!* **Мут на 20 минут**".format(ctx.message.author.name), color = 0xffff00)
